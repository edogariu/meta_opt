...
from .controllers.utils import append, index_pytree, add_pytrees, multiply_pytrees, multiply_pytree_by_scalar
...
def update(cstate,
    ...
    (loss, (tstate, disturbances, emas)), grads = jax.value_and_grad(_compute_loss, (0,), has_aux=True)(cstate.cparams, tstate, batch, disturbances, emas, cstate.H, cstate.HH)
...
class MetaOpt:
    ...
    def step(self, tstate, batch):  
        ...
        if self.t >= self.cstate.H:
            self.cstate, (tstate, loss, self.grad_history, self.emas) = update(self.cstate, tstate, batch, self.grad_history, self.emas)
        else: 
