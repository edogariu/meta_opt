...
import meta_opt.workloads._wmt.tokenizer as tokenizer
...
def get_raw_dataset(
  ...
  Returns:
    ...
    'targets'.
  """
  if split == 'train': ds = tfds.load(config.dataset_name, split='train', data_dir=vocab_path)
  elif split == 'test': ds = tfds.load(config.eval_dataset_name, split=config.eval_split, data_dir=vocab_path)
  else: raise NotImplementedError(split)
...
def get_wmt_datasets(
  ...
  train_data = get_raw_dataset(
      train_ds_builder, 'train', reverse_translation=reverse_translation, vocab_path=vocab_path, config=config,
  )
