...
class MetaOptConfig(OptimizerConfig):
    ...
    freeze_loss_fn_during_rollouts: bool  # whether to use one fixed batch during counterfactual rollouts
    ...
    def make_jax(self) -> optax.GradientTransformationExtraArgs:
        ...
        opt = make_jax_metaopt(base_lr=self.initial_learning_rate, weight_decay=self.weight_decay, grad_clip=self.grad_clip, scale_by_adam_betas=self.scale_by_adam_betas,
                               ...
                               fake_the_dynamics=self.fake_the_dynamics, freeze_gpc_params=self.freeze_gpc_params, freeze_loss_fn_during_rollouts=self.freeze_loss_fn_during_rollouts)
        return opt
...
def update_gpc_controller_counterfactual(gpc_params: chex.Array,
                                         ...
                                         loss_fn_history,  # past HH loss functions, starting at the one that would have been used to evolve `initial_params`
                                         curr_loss_fn,
                                         unflatten_fn,  # static
    ...
    def gpc_loss_fn(controller_params: chex.Array):
        ...
        for h in range(HH):
            if fake_the_dynamics:
                ...
                grads = jax.grad(lambda p: loss_fn_history[h](unflatten_fn(p)))(params)
    ...
    return gpc_params, new_gpc_opt_state, gpc_loss, gpc_grads
...
class JaxMetaOptState(struct.PyTreeNode):
    ...
    loss_fn_history: Tuple[Callable] = struct.field(pytree_node=True)
...
def make_jax_metaopt(
        ...
        freeze_loss_fn_during_rollouts: bool,
    ...
    Args:
        ...
        freeze_loss_fn_during_rollouts: bool
            whether to use the same loss function (i.e. same batch) during the entire rollout
    """
    ...
    def init_fn(params):
        ...
        if freeze_gpc_params:
            ...
            if fake_the_dynamics: 
                assert not freeze_loss_fn_during_rollouts, 'if we are faking the dynamics we need to receive the true loss functions'
    ...
    def update_fn(grads: chex.ArrayTree, 
                  ...
                  loss_fn: Callable[[chex.ArrayTree], float],
                  **kwargs,
                  ):
        ...
        Args:
            ...
            loss_fn (Callable[[chex.ArrayTree], float]): autodifferentiable function sending `params -> loss`, whose gradient evaluated at `params` should be `grads`
        ...
        if not freeze_gpc_params:
            ...
            def true_fn(opt_state, base_lr, weight_decay, loss_fn, H, HH, fake_the_dynamics):
            ...
            def false_fn(opt_state, base_lr, weight_decay, loss_fn, H, HH, fake_the_dynamics):
                return opt_state.gpc_params, opt_state.gpc_opt_state, opt_state.recent_gpc_loss, opt_state.recent_gpc_grads
            gpc_params, gpc_opt_state, gpc_loss, gpc_grads = jax.lax.cond(opt_state.t >= H + HH, true_fn, false_fn, opt_state, base_lr, weight_decay, loss_fn, H, HH, fake_the_dynamics)
