# Meta-Optimization
Hi! This is the codebase for our [meta-optimization algorithm](https://openreview.net/pdf?id=AJwlILrBOr), built atop the nonstochastic control theory. We provide an implementation as an `optax` optimizer (found in the `jax_meta_opt` function in `meta_opt/jax_stuff/jax_meta_opt.py`) and plan to release a `pytorch` version soon.
